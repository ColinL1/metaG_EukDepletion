...
process CAT_NR {
    tag "${meta.id}"
    publishDir "$baseDir/results/NR/${meta.species}/${meta.id}/", mode: 'symlink'
...
process CAT_GTDB {
    tag "${meta.id}"
    publishDir "$baseDir/results/GTDB/${meta.species}/${meta.id}/", mode: 'symlink'
...
workflow {
    Channel.fromPath("/home/colinl/metaG/Git/metaG_EukDepletion/manual_piplines/CAT/input_coassembled/*.fa")
