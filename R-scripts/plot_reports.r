...
# sample_data["sample"] <- paste(sample_data$Strain, sample_data$Replicate, sample_data$Origin, sample_data$DATA_Type, sep = "-") 
# sample_data["sample_short"] <- paste(sample_data$Replicate, sample_data$Origin, sample_data$DATA_Type, sep = "-") 

# sample_data["identifier"] <- paste(sample_data$Strain, sample_data$Replicate, sample_data$Treatment, sample_data$Buffer, sample_data$DATA_Type, sample_data$Origin, sample_data$Type, sep = "_") 
...
# # # 
# # sample_data["sample"] <- paste(sample_data$Strain, sample_data$Replicate, sample_data$Origin, sep = "_") 


# sample_df <- sample_data_2[,c("file_name", "Strain", "Replicate", "Treatment", "Buffer", "DATA_Type", "Origin", "sample", "sample_short", "total", "sequenced_total", "Total_percentage", "Kingdom", "Type")]
...
sample_data["sample"] <- paste(sample_data$Strain, sample_data$Replicate, sample_data$Origin, sample_data$DATA_Type, sep = "-") 
sample_data["sample_short"] <- paste(sample_data$Replicate, sample_data$Origin, sample_data$DATA_Type, sep = "-") 
sample_data["sample_short"] <- paste(sample_data$Replicate, sample_data$Origin, sep = "-") 
...
Acro_contigs <- sample_df %>%
        ...
        panel.grid.minor = element_blank(),
        axis.text.x = element_text(angle = -0, hjust = 0.5, vjust = 1)) + 
        ggtitle( "Acropora - contigs illumina") +
        scale_x_discrete(labels = function(x) str_wrap(x, width = 1, whitespace_only = FALSE))
...
Poci_contigs <- sample_df %>%
        ...
        panel.grid.minor = element_blank(),
        axis.text.x = element_text(angle = -0, hjust = 0.5, vjust = 1)) + 
        ggtitle( "Pocillopora - contigs illumina") +
        scale_x_discrete(labels = function(x) str_wrap(x, width = 1, whitespace_only = FALSE))
