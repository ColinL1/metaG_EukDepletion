...
# # Load ggtext for element_markdown
...
color_palette_x <- c("#9AE0EC", "#87ACE9", "#9C80BA")

# # Families categorized
# intracellular <- c(
#     "Hyphomicrobiaceae", "Phyllobacteriaceae", "Sphingomonadaceae",
#     "Roseobacteraceae", "Rickettsiaceae", "Burkholderiaceae",
#     "Microbacteriaceae", "Paracoccaceae", "Planctomycetaceae", "Pirellulaceae"
# )

# closely_associated <- c(
#     "Hyphomonadaceae", "Phycisphaeraceae", "Flavobacteriaceae",
#     "Marinobacteraceae", "Oceanospirillaceae", "Thiotrichaceae",
#     "Cellvibrionaceae", "Endozoicomonadaceae", "Nitrobacteraceae",
#     "Lacipirellulaceae"
# )

# other <- c(
#     "Spirulinaceae", "Leptolyngbyaceae", "Desertifilaceae", "Sedimenticolaceae",
#     "Schleiferiaceae", "Marinifilaceae", "Prevotellaceae", "Vibrionaceae"
# )
...
for (i in sort(unique(CAT_data_family_curated$Strain))) {
    print(i)
        data_to_plot <- CAT_data_family_curated %>%
            ...
            filter(grouping != "H2 - DESS") %>%
            filter(grouping != "F003 - DESS")
            # filter(grouping != "H2 - PBS") %>%
            # filter(grouping != "F003 - PBS")

        # # Custom function for y-axis labels
        # family_labeller <- function(y) {
        #     sapply(y, function(fam) {
        #     if (fam %in% intracellular) {
        #         # Red and bold
        #         as.expression(bquote(phantom(bold("x")) * bold(.(fam))))
        #     } else if (fam %in% closely_associated) {
        #         # Bold only
        #         as.expression(bquote(bold(.(fam))))
        #     } else {
        #         fam
        #     }
        #     })
        ...
        p <- data_to_plot %>%
            ...
            axis.text.y = element_markdown(size = 12) # for ggtext if you want color
            ) +
            scale_x_discrete(labels = function(x) str_wrap(x, width = 15)) #+
            # scale_y_discrete(labels = function(y) {
            # sapply(y, function(fam) {
            #     if (fam %in% intracellular) {
            #     # Red and bold (using HTML, requires ggtext)
            #     paste0("<span style='color:red; font-weight:bold;'>", fam, "</span>")
            #     } else if (fam %in% closely_associated) {
            #     # Bold only
            #     paste0("<b>", fam, "</b>")
            #     } else {
...
legend <- extract_legend(plot_family_list_dot[[1]] + theme(legend.position = "right", legend.key.size = unit(1.5, "lines"), legend.title = element_text(size = 16) ) + guides(fill = guide_legend(nrow = 1))) 

figure_4_dot <- ggarrange(
                # ggarrange(
...
for (i in c("H2", "F003")) {
    print(i)
        data_to_plot <- CAT_data_family_curated %>%
            ...
            # filter(grouping != "H2 - PBS") %>%
            # filter(grouping != "F003 - PBS")
        data_to_plot$Strain <- factor(data_to_plot$Strain, levels = rev(c("F003", "H2")))
        p <- data_to_plot %>%
            ggplot(aes(x = Buffer, y = Family, color = Treatment)) +
            geom_point(aes(size = log10(total_contigs) + 1), shape = 16, show.legend = TRUE) +
            facet_grid(Strain ~ Treatment ) + #, ncol = 3) +
            labs(title = "", x = "", y = "") +
            # scale_color_manual(values = color_palette_x) +
            theme_minimal(base_size = 16) +
            theme(
            legend.position = "bottom",
            legend.box = "vertical",  # Stack legends vertically (one on top of the other)
            panel.grid.minor = element_blank(),
            strip.text.x = element_text(size = 12, face = ifelse(i %in% c("F003", "H2"), "plain", "italic")),
            axis.text.x = element_text(
                size = 12,
            ...
            scale_x_discrete(labels = function(x) str_wrap(x, width = 15))
p
    plot_family_list_dot_aip[[i]] <- p

}
plot_family_list_dot_aip

ggarrange(
    plot_family_list_dot_aip[[1]],
    plot_family_list_dot_aip[[2]],
    ncol = 1, nrow = 2
)
# CAT_data_family_curated %>%
#         # filter(Strain == i) %>%
...
ggsave(filename = paste0(out_path, "/figure_4_dot.pdf"), plot = figure_4_dot, width = 14, height = 16)
